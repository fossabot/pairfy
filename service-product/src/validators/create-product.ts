...
export const createProductSchema = z.object({
  name: z.string().min(1).regex(productNameRegex, "Invalid product name"),
  price: z.number().int().nonnegative().refine((val) => productPriceRegex.test(val.toString()), {
    message: "Invalid price format",
  }),
  sku: z.string().min(1).regex(skuRegex, "Invalid SKU format"),
